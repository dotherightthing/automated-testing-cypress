...
describe('Search modal', function () {
  describe('Default view', function () {
    ...
    it('UI displays the correct elements', function () {
      ...
      /* fails due to flakey state
      cy.get('.b-modal-js .l-ajax-js__inner').as('ajaxSpinner');
      cy.get('@ajaxSpinner').should('have.class', 'l-ajax-js__inner--in').should('be.visible');
      */

      // check that the search theming has been applied
      cy.get('.b-modal-js').as('modal');
      cy.get('@modal').should('have.class', 'b-modal-js--search');
      ...
      /* TODO: how to check that non-existent element doesn't exist?
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count')
        .as('resultsCount');
      cy.get('@resultsCount').should('not.exist');
      */

      /* TODO: how to check that non-existent element doesn't exist?
      cy.get('.b-modal-js #search-results-summary .b-no-results-message').as('noResultsMessage');
      cy.get('@noResultsMessage').should('not.exist');
      */
      ...
      cy.get('@searchSuggestionsAndHelp').should('be.visible');
    ...
    it('UI displays a typeahead when 3 characters are entered', function () {
      ...
      cy.get('.b-modal-js #search-modal').as('searchField');
      cy.get('@searchField').type('ea').should('have.value', 'Lea');

      // typing three letters should launch the typeahead
      cy.get('.b-modal-js #search-modal_listbox').as('typeaheadResults');
      cy.get('@typeaheadResults').should('be.visible');

      cy.get('.b-modal-js .b-tt-js__suggestion').as('typeaheadResult');
      cy.get('@typeaheadResult').should('have.length', 10);

      // arrow down arrow to the 3rd suggestion and select it
      cy.get('@searchField').type('{downarrow}{downarrow}{downarrow}');

      // check that the 3rd item is highlighted
      cy.get('@typeaheadResult').eq(2)
        .should('have.text', 'UDL (Universal Design for Learning)').should('have.class', 'b-tt-js__cursor');

      // pressing enter should put the typeahead result into the search field and hide the typeahead
      cy.get('@searchField').type('{enter}').should('have.value', 'UDL');
      cy.get('@typeaheadResults').should('not.be.visible');
    });
  });
  ...
  describe('Results view (No Results with suggestions)', function () {
    it('UI shows no matching results', function () {
      // check that the spinner appears
      cy.get('.b-modal-js .l-ajax-js__inner').as('ajaxSpinner');
      cy.get('@ajaxSpinner').should('be.visible');
      ...
      cy.get('@filterRadio1').should('have.value', 'all');
      cy.get('@filterRadio1').should('be.checked');

      // check that the summary is output
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count').as('resultsCount');
      cy.get('@resultsCount').contains('0 results for “balloons”');
    ...
    it('User can run a popular search', function () {
      ...
      cy.get('@popularSearches').find('button').contains('Bullying').click();
      ...
      */
      ...
      cy.get('.b-modal-js #search-modal').as('searchField');
      cy.get('@searchField').should('have.value', 'Bullying');
  ...
  describe('Results view (Popular Search Results)', function () {
    it('UI hides irrelevant content', function () {
      ...
      cy.get('.b-modal-js .b-search-suggestions, .b-modal-js .b-search-suggestions__heading, .b-modal-js .b-search-result').as('searchSuggestionsAndHelp');
      cy.get('@searchSuggestionsAndHelp').should('not.be.visible');
    });
    ...
    it('UI displays relevant filters', function () {
      ...
      cy.get('@filter').should('be.visible');
      cy.get('@filter').should('have.attr', 'data-content-filter-totals', '{"all":64,"guides":40,"suggestions":20,"resources":3,"other":0}');

      // check that the first filter is selected
      cy.get('.b-modal-js #search_filter_1').as('filterRadio1');
      cy.get('@filterRadio1').should('have.value', 'all').should('be.checked');
      ...
      cy.get('@filterNotOther').should('not.be.disabled');

      /*
      cy.get('.b-modal-js .b-guide-list-search-and-filter .b-filter__input[value="other"]')
        .as('filterOther').should('be.disabled');
      */
    });
    ...
    it('UI displays relevant results summary', function () {
      ...
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count').as('resultsCount');
      cy.get('@resultsCount').contains('64 results for');
      cy.get('@resultsCount').contains('Bullying');
  ...
  describe('Results view (Guides Filter - Synonymical Term Search Results)', function () {
    ...
    it('UI displays relevant results message', function () {
      ...
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count').as('resultsCount');
      cy.get('@resultsCount').contains('40 Guides match “Bullying”').contains('Child Aggression Syndrome');
    });
    ...
    it('User can search on a canonical term', function () {
      ...
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count').as('resultsCount');
      cy.get('@resultsCount').find('[data-field-suggestion] .b-button__content').as('resultSummarySuggestionButton');
      cy.get('@resultSummarySuggestionButton', { multi: true }).click();

      // check that the suggestion appears in the search field
      // and this causes the clear button to remain visible
      cy.get('.b-modal-js #search-modal').as('searchField');
      cy.get('@searchField').should('have.value', 'Child Aggression Syndrome');
  ...
  describe('Results view (Guides Filter - Canonical Term Search Results)', function () {
    it('UI should display the correct elements', function () {
      ...
      cy.get('@resultsCount').find('[data-field-suggestion] .b-button__content').as('resultSummarySuggestionButton');
      cy.get('@resultSummarySuggestionButton').should('have.length', 0);
  ...
  describe('Results view (Resources Filter - Search Results)', function () {
    it('UI displays relevant filters', function () {
      ...
      */
      ...
      // summary total should be updated
      cy.get('.b-modal-js #search-results-summary .b-search-results-summary__count').as('resultsCount');
      cy.get('@resultsCount').contains('3 Resources match “Child Aggression Syndrome”');
    ...
    it('Hide-show is set up correctly', function () {
      ...
      cy.get('@revealTrigger5').should('not.have.class', 'is-opened');

      cy.get('#search-result-expand-5').as('revealTarget5');
      cy.get('@revealTarget5').should('have.attr', 'data-hidden');

      // clicking should show the hideshow
      cy.get('@revealTrigger5').click().should('have.class', 'is-opened');
      cy.get('@revealTarget5').should('not.have.attr', 'data-hidden');

      // clicking should hide the hideshow
      cy.get('@revealTrigger5').click().should('not.have.class', 'is-opened');
      cy.get('@revealTarget5').should('have.attr', 'data-hidden');
    });
    ...
    it('User can use load more to load more results', function () {
      ...
      // check that the pagination button is output
      cy.get('.b-modal-js #search-results-pagination-button').as('pagination');
      cy.get('@pagination').scrollIntoView().contains('Load 1 more result');

      // TODO check that the button spinner appears

      // click the load more button
      cy.get('@pagination').click().should('not.be.visible');
    ...
    it('Show-hide is set up correctly', function () {
      ...
      cy.get('@revealTrigger6').should('have.class', 'is-opened');

      cy.get('#search-result-expand-6').as('revealTarget6');
      cy.get('@revealTarget6').should('not.have.attr', 'data-hidden');

      // clicking should hide the hideshow
      cy.get('@revealTrigger6').click().should('not.have.class', 'is-opened');
      cy.get('@revealTarget6').should('have.attr', 'data-hidden');

      // clicking should show the hideshow
      cy.get('@revealTrigger6').click().should('have.class', 'is-opened');
      cy.get('@revealTarget6').should('not.have.attr', 'data-hidden');
    });
  });
});
